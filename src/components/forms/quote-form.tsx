...
import { submitQuoteForm } from '@/lib/actions/quote';
...
export function QuoteForm() {
  ...
  const [state, formAction, pending] = useActionState(submitQuoteForm, initialState);

  function getFieldError(field: keyof QuoteFormData) {
  ...
  async function onSubmit(data: QuoteFormData) {
    const token = executeRecaptcha ? await executeRecaptcha('quote') : '';
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          {errors.destinationCountry && (
            <p className="mt-1 text-sm text-destructive">{getFieldError('destinationCountry')}</p>
          )}
        </div>
      </div>
      ...
      <div>
        <Label htmlFor="details">
          {t('details')} <span className="text-muted-foreground font-normal">({tOptional('optional')})</span>
        </Label>
        <Textarea
          id="details"
