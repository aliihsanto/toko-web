...
import { sourcingSchema } from '@/lib/schemas/sourcing';
import { sendNotificationEmail } from '@/lib/email/resend';
import { rateLimit } from '@/lib/rate-limit';
import { verifyRecaptcha } from '@/lib/recaptcha';
import type { FormState } from './types';

export async function submitSourcingForm(
  prevState: FormState,
  formData: FormData
): Promise<FormState> {
  ...
  const result = sourcingSchema.safeParse(rawData);
  if (!result.success) {
    return {
      success: false,
  ...
  // 5. Send email
  try {
    await sendNotificationEmail('sourcing', result.data);
    return { success: true, message: t('success') };
  } catch {
